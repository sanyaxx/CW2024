 ...
 *<p>
 * Secondly, `LevelParent extends Observable` has been removed as the logic has been shifted to levelManager class where
 * LevelManager implements PropertyChangeSupport has been used instead-since Observable was deprecated.
 * </p>
 */
public abstract class LevelParent implements Updatable {
	...
	/** The audio handler managing audio controls */
		...
		inputHandler.setupInputHandlers(
				...
				() -> {
					if (!user.isCollisionCooldownActive()) { // Check if cooldown is not active
						...
						audioHandler.playProjectileFiredSound(); // Play fire projectile sound
